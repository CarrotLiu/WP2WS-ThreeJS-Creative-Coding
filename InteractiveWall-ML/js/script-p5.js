...
let mlHand,
  handCenter,
  ifHandMove = false,
  handArea = { xMin: 0, xMax: 0, yMin: 0, yMax: 0 };
let handpos;
let predictions = [];
let handPosition;
...
function setup() {
  ...
  cam = createCapture(videoOptions);
  mlHand = new HandArea();

  handpos = ml5.handpose(cam, modelLoaded);
  handpos.on("hand", (results) => {
    predictions = results;
...
function draw() {
  ...
  if (predictions.length != 0) {
    handPosition = {
      thumb: predictions[0].landmarks[1],
      indexFinger: predictions[0].landmarks[5],
      middleFinger: predictions[0].landmarks[9],
      ringFinger: predictions[0].landmarks[13],
      pinky: predictions[0].landmarks[17],
      palmBase: predictions[0].landmarks[0],
    };
    handCenter = mlHand.onHandMove(handPosition);
    // console.log(handPosition);
    handArea.xMin = handCenter.x - 5;
    handArea.xMax = handCenter.x + 5;
    handArea.yMin = handCenter.y - 5;
    handArea.yMax = handCenter.y + 5;
    ifHandMove = true;
  }
